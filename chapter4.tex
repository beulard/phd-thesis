...
This section describes how the simulated hits are selected and arranged, and how the networks are consequently built in order to learn a representation of the data. Multiple aspects of the problem must be considered carefully in order for the GAN to produce a faithful model, such as how to select, lay out and pre-process the hit data, and how to emphasise the importance of relationships between consecutive hits during training.
...
\begin{table}[]
    ...
    \hline
         & Energy deposit & Time & Distance of closest approach & Wire index  \\\hline
        Units & \SI{}{\MeV} & \SI{}{\ns} & \SI{}{\mm} & N/A \\
        Label & \texttt{edep} & \texttt{t} & \texttt{doca} & \texttt{wire} \\
        Data type & \texttt{float} & \texttt{float} & \texttt{float} & \texttt{long}
        \\\hline
    \end{tabular}
...
Monte Carlo data and GAN-generated data are inherently different. Monte Carlo simulation provides full truth information about every hit in the detector system, including the type and whereabouts of the particles that produced them.
...
In contrast, the GAN is not explicitly aware of physics. Without a conditioning mechanism, it is impossible to extract particle type or track momentum from synthetic hits.
However, not all hits in the detector require associated truth information, and in fact the majority of hits in the CDC appear like background noise whose origin is irrelevant to any reconstruction procedure. % The question is: DEFINE EXACTLY WHAT KIND OF DATA NEEDS TRUTH
