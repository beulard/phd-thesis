...
\subsection{Hit data structure}\label{sec:hit_data_structure}
...
Before training, the three continuous hit features (energy deposit, time and DCA) undergo a non-linear \emph{quantile transformation}. Each feature's cumulative distribution function is estimated to map the values to a uniform distribution. Then, the new values are mapped to a normal distribution using the quantile function. 

Transforming our data such that it is normally distributed helps the GAN learn the distributions of each feature and the correlations between them, as is shown in Section~\ref{sec:gan_eval}. In addition, since it is common for the final activation layer of the generator to be a hyperbolic tangent function, each feature is also re-scaled independently such that their values lie in the same range as $\tanh$, i.e.\ $[-1, 1]$.
...
The fourth feature, wire index, is not modified. However, it receives a special treatment during training because it encodes the hit positions, which is an important feature that both the generator and discriminator must process appropriately.

\subsection{Hit sequence sampling and data augmentation}
At each step of the training, an entire sequence of $L$ hits\footnote{More accurately, an entire mini-batch of $N$ sequences of $L$ hits.} is sampled and passed through the discriminator to calculate its output and loss function. As discussed in Section~\ref{sec:hit_data_structure}, the order in which hits are laid out matters, because the hits produced by a given particle occur consecutively in the simulation (hence they are interrelated). 

For the GAN model to be efficient, $L$ is preferably fixed such that fast tensor operations can be applied to the hit sequence.
Hence, $L$ becomes a hyperparameter of the training, and may be smaller or greater than the typical number of hits in one MC event. In the latter case, hits from multiple events are concatenated to form a sequence, and some hits from the last event may be deferred to the next sequence if the total number of hits exceeds $L$. 

Hits belonging to different events are completely unrelated. This provides freedom to swap events and concatenate their hits in a new order. The order of hits within the same event is preserved, but the boundaries are modified. % as in: last hit of event X leads to a different hit in event Z than in event Y
If the sequence length $L$ is significantly greater than the typical number of hits per event, then this event recombination can greatly enhance the diversity of the training dataset.

The training dataset obtained from MC5 contains \numprint{49101} unique, independent events. On average, one event contains 13 hits. Assuming a sequence length of $L=512$, 40 events are required to compose one sequence. Hence the number of possible combinations is ${49101 \choose 40} \approx 10^{139}$. Such a large number of samples is unnecessary to train the GAN, thus we select a subset of \numprint{500000} sequences as our training dataset.
...
\subsection{Evaluation}\label{sec:gan_eval}
