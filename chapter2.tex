...
COMET is planned to run in a staged approach so that the properties of the
newly designed beam can be finely understood before making the measurement.
COMET Phase\nobreakdash-I has a double purpose, each fulfilled by a distinct detector
...
single-event sensitivity (see Section~\ref{sec:SES}) of $3.1\times 10^{-15}$, a
factor-100 improvement over SINDRUM II. COMET Phase\nobreakdash-II is a planned upgrade to
Phase\nobreakdash-I with higher beam intensity and better background rejection via a longer
momentum-selecting beamline. Phase\nobreakdash-II aims to improve on the single-event
sensitivity of Phase\nobreakdash-I by another factor $100$ to reach a single event
...
Phase\nobreakdash-I and Phase\nobreakdash-II. In the latter, the transport solenoid is extended to allow
...
signature before they enter the detector system. While Phase\nobreakdash-I will use the
StrECAL to study the beam properties, Phase\nobreakdash-II will use it as the
...
by the J-PARC Main Ring synchrotron. Protons are delivered with an energy of
\SI{8}{\GeV}, which is chosen to optimise between a high pion yield and a low
yield of anti-protons, which are a potential background source.
...
\SI{1170}{\ns}, and every fifth bunch is delayed by an additional
...
\begin{figure}
    ...
    \caption{ 
        Timeline of a COMET event. Shortly after the proton collision, the
        beam flash floods the detector and muons begin to be bound in the muon
        stopping target. The Phase-I trigger window shown here
        starts at $t=\SI{700}{\ns}$ and lasts until the next proton bunch
        collision. Electron tracks with a momentum around \SI{105}{\MeV/\clight}
        observed within this timing window are considered as signal candidates.
        }
...
graphite in Phase\nobreakdash-I, and tungsten in Phase\nobreakdash-II. This region is permeated by a
...
is positioned in the opposite direction to the proton beam such that only
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{chapter2/pion_mom-v2.pdf}
    \caption{ Momentum distribution of pions produced by simulating proton
    collisions with Geant4~\cite{AGOSTINELLI2003250}, depending on whether their
    initial direction is forward or backward with respect to the proton beam
    direction. }
...
The transport solenoid is a curved magnet connecting the pion-production section
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \hspace{-0.8cm}
        \includegraphics[width=\textwidth]{chapter2/drift_vs_s_0mT_with_charge.pdf}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \hspace{-0.8cm}
        \includegraphics[width=\textwidth]{chapter2/drift_vs_s_0.05mT_with_charge.pdf}
        \caption{\SI{0.05}{\tesla} vertical dipole field.}
    \end{subfigure}
    \caption{ Effective drift of particles of various momenta and either charge
        as they progress along the Phase-I transport solenoid, calculated using
        Equation~\ref{eq:drift}. The pitch angle is assumed to be
        \SI{45}{\degree} and the bands show  the effect of a $\pm
        \SI{10}{\degree}$ difference. 
        As shown on the right, a \SI{0.05}{\tesla} dipole field allows negative
        particles around \SI{45}{\MeV/\clight} to stay on axis while higher- and
        lower-momentum particles drift in opposite directions and positive
        particles all drift one way. By use of collimators at the end of the
        transport section, only particles of the right momentum and charge are
        allowed through. }
    \label{fig:drift}
\end{figure}
...
\begin{figure}
    ...
    are shown in blue and the electromagnetic calorimeter (ECAL) in yellow.  }
    \label{fig:strecal}
\end{figure}

The StrECAL combines a straw-tube tracking detector with an electromagnetic
calorimeter for energy measurement. In COMET Phase\nobreakdash-I, the StrECAL will serve as
...
In COMET Phase\nobreakdash-II, the StrECAL will serve as the detector system for the
...
the StrECAL detector system in Phase\nobreakdash-II.

The Straw Tracker uses long, thin-walled polyethylene terephthalate tubes as
gaseous drift chambers. The tubes are first arranged into circular planes, four
of which constitute a station. Each station is able to measure the horizontal
and vertical position of a particle and has a spatial resolution of
\SI{120}{\um}. Using time-of-flight information, the Straw Tracker can therefore
reconstruct trajectories and estimate particle momenta.
...
reaching the chamber. In order to achieve the sensitivity goal for Phase\nobreakdash-I, the
...
% Geometry
The CDC contains 4986 \emph{sense wires} strung out parallel to its axis in 20
concentric layers. The wires in each layer are rotated slightly off from the
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[height=6.4cm]{chapter2/signal_event_display_crop_axes_margin.pdf}
    ...
    \begin{subfigure}[b]{0.53\textwidth}
        \centering
        \includegraphics[height=6.5cm]{chapter2/cydet_signal_track_v4.pdf}
...
\label{sec:crv}
The COMET experiment hall, being at sea level, is constantly irradiated by muons
produced in the atmosphere by cosmic rays. The cosmic ray veto (CRV) is an
additional active detector which will tightly enclose the CyDet and StrECAL
detector systems. Its purpose is to identify events induced by atmospheric muons
rather than by the COMET beam, and thus reduce the probability that such events
will be mistaken for a conversion signal.
...
Figure~\ref{fig:timing_distributions}. In Phase\nobreakdash-I, in order to suppress prompt
...
intense beam as well as cosmic rays.

Beam-induced backgrounds include delayed events caused by slow particles and
products of stray protons arriving between two bunches. Antiprotons are the main
source of delayed backgrounds due to their slow speed relative to pions and
muons of the same momentum. They are negatively charged, therefore they cannot
be efficiently filtered out by the beamline. Hence, they can move slowly toward
the muon stopping target and produce signal-like secondary electrons during the
trigger window.

Stray protons, i.e.\ protons from the proton beam that arrive at the
pion-production target in the interval between bunches, can produce secondary
particles that reach the detector during the trigger window. To reduce the
background rate from this type of event, COMET requires a strict extinction
factor $R_\mathrm{extinction} = 10^{-10}$ from the J-PARC beam.

Finally, cosmic rays are a major source of background events. The background
rate depends heavily on the amount of shielding and material above the COMET
detector system, as well as the efficiency of the cosmic ray veto. The topic of
rate estimation for cosmic ray-induced backgrounds is discussed more thoroughly
in Chapter~\ref{ch:cosmics}.



Table~\ref{tab:backgrounds} shows the rates estimated in the COMET Phase\nobreakdash-I
...
data acquisition run time is predicted to be $N_\mathrm{background} = 0.032$
given an extinction factor $R_\mathrm{extinction} = 3,1 \times 10^{-11}$. Since
\mbox{$N_\mathrm{background} \ll 1$}, the observation of a single signal
electron would suggest that there is a CLFV process at play, which would further
motivate this search through COMET Phase\nobreakdash-II and beyond.
...
background events is less than one. 
...
\end{equation}
In COMET Phase\nobreakdash-I, the signal acceptance can be broken down into seven efficiency
...
COMET Phase\nobreakdash-I TDR~\cite[Section 10.1]{the_comet_collaboration_comet_2020}. 
...
goal of the experiment, $\mathrm{SES} = 3.1\times 10^{-15}$, using
...
for a Phase\nobreakdash-I SES of $3.1 \times 10^{-15}$.
